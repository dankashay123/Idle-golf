...
| `render` | Every course in every weather and every Depths floor draws, and the ground pass stays in budget. Themes are authored as literal tables in places, so a colour the renderer newly depends on can be missing on exactly one course — which is how the Depths went down on its first frame once. Also that nothing draws a hard line across the frame: the sky and the ground have to meet at the horizon without a step, and the haze over them has to keep its shape on a tall buffer as well as a short one. |
