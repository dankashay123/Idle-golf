  ...
  async run(page) {
    const r = await page.evaluate(() => {
      ...
      try {
        ...
        o.own = {};
        for (const id of ['divine', 'demonic', 'ascended']) {
          wear(id); Scene.moveT = undefined; Scene._lastMove = undefined; Scene._moveRnd = undefined; Scene.fairyMove = null;
        ...
        // ---- the developer menu ----
        o.dev = FAIRY_MOVES.map(m => { wear(m.only || 'bib'); Scene.fairyMove = null; DEV.move(m.id); return Scene.fairyMove && Scene.fairyMove.kind; }).join();
        wear('bib');
    ...
    if (/soar|blaze|blink/.test(r.kinds)) f('a plain caddie took a dearest caddie\'s trick: ' + r.kinds);
    const OWN = { divine: 'soar', demonic: 'blaze', ascended: 'blink' };
    for (const id in OWN) if (r.own[id] !== OWN[id]) f('in twenty minutes the ' + id + ' caddie\'s own tricks were "' + r.own[id] + '", not ' + OWN[id]);
    if (r.plainSoar) f('a plain caddie could be made to soar');
    ...
    return ['no wings: idle he stays in his box and in place across, floating ' + r.yRange + 'px up and down',
      ...
      'the dearest caddies\' own: ' + Object.entries(r.own).map(([k, v]) => k + ' ' + v).join(', ') + ', no one else\'s'];
  }
};
